...
// 로그인 안 되어 있으면 Servlet으로 요청을 보내지 않고 로그인 페이지로 redirect.
...
public class AuthenticationFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// 로그인되어 있는 지를 체크. - 세션에 로그인 정보(signInUser) 가 있는 지를 확인  //(signInUser로 해서 정보를 저장함.)
        ...
        if(signInUser != null) { //로그인 정보가 있다면
            ...
            return;   // 메서드 종료     ---(필터 할 일이 끝났기 떄문에)
        }
        log.info("로그아웃 상태 -> 로그인 페이지로...");
        // 로그인 페이지로 이동(reditect)
