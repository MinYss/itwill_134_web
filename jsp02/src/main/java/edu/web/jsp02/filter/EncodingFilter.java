 ...
 */
@Slf4j
@WebFilter(filterName = "encodingFilter", urlPatterns = { "/*" }) //  /* : 서버에 들어오는 모든 요청에 대해서 filtering하겠다는 뜻
public class EncodingFilter extends HttpFilter implements Filter {
    ...
    @Override
	public void destroy() {
		// Ffilter 객체가 소멸될 때 WAS에서 호출하는 메서드. (WAS가 종료될 때. 서버가 실행될 때는 실행되지 않음.)
        log.trace("destory() 호출");
	}
