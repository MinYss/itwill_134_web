...
@Slf4j
@Controller  // 컴포넌트 에너테이션 중 하나. 웹 요청을 처리하는 Controller계층의 컴포넌트.
// -> @Controller 애너테이션 + sevlet-context.xml 설정 <context:component-scan>
// -> 스프링에서 객체를 생성해서 메모리에 로딩하고 관리. -> 객체의 메서드들을 필요할 떼 호출.
public class ExampleController {
    ...
    // (2) 컨트롤러 메서드가 문자열을 리턴하지 않는 경우(void), 요청 주소 이름이 뷰의 이름.
    ...
    public void ex2(String username, int age, Model model) {
        ...
        // Model 타입 갹체: 컨트롤러에서 뷰로 전달할 데이터가 있을 때 사용.
        // -> model 속성(attribute)이름을 JSP에서 EL로 사용할 수 있음.
    ...
    public String ex3(@RequestParam(name="username") String name,  //(1)
        ...
        // (1) 메서드 파라미터와 요청 파라미터 이름이 다를 때, name속성 설정.
    ...
    public String ex4(UserDto user, Model model) {
        log.info("ex4(){}") ;
        // DispatcherServlet은 
        // 1. 요청 파라미터들을 분석(request.getParmeter).
        // 2. UserDtp의 기본 생성자를 호출해서 객체를 생성,
        // 3. 요청 파라미터들의 이름을 Username setter 메서드를 찾아서 호출.
