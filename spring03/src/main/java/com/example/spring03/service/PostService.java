...
public class PostService {
    ...
    @Transactional(readOnly = true)   // 내용을 변경할게 아니라면 readOnly = true를 해주면 검색 속도가 빨라진다.
    ...
    // delete는 변경이 일어나기 때문에 readOnly를 안해줌
    ...
    @Transactional   //readOnly == false(기본값) -- 변경되기 때문에 true를 안 붙여준 것.
    public Integer update(PostUpdateDto dto) {
        log.info("update(dto={}", dto);
        
        // 메서드에 @Transactional 에너테이션을 사용하고,
        // 수정하기 전의 엔터티 객체를 검색한 후에
        // 검색된 엔터티 객체를 수정하면
